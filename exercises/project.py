...
def newMenuItem(restaurant_id):
    ...
    if request.method == 'POST':
        newItem = MenuItem(name=request.form['name'], restaurant_id=restaurant_id) #restaurant.id?
        session.add(newItem)
        session.commit()
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if request.method == 'POST':
    	...
    	if request.form['new_name']:
    		editedItem.name = request.form['new_name']
...
#create route function for JSON API endpoint
